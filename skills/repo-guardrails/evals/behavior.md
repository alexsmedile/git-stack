...
| G06 | TEAM: guard opening and marking a PR ready for review; the provider supports required conversation resolution. |
...
| O01 | In an audit, TEAM requires review; an adopted local policy allows a docs-only bypass and records its source, scope, rationale, and expiry. |
| O02 | In an audit, two equal-authority files disagree on approvals. |
